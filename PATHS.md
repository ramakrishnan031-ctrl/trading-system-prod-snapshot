...
SHADOW→ACTIVE only when the flag is on; SYMMETRIC LONG+SHORT (ONE flag, ONE set of side-neutral knobs; NO schema change — direction already persisted). Phase B (SL-reposition / structure-break exit) parked. branch `snr-v2-phaseA-27jun` (on V1), NOT pushed; schema awaiting nod. Detail: `docs/SYSTEM_MAP.md` header · memory `snr_v2_phaseA_27jun`.
