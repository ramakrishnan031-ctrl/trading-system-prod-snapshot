...
(`ubuntu@80.225.198.195`). This file is the single point of truth for the
...
1. Rotate Zerodha credentials (Kite dev console) + SCP fresh `.env`:
   ```bash
   scp -i ~/.ssh/trading_vm_secure .env ubuntu@80.225.198.195:~/trading-system/.env
   ```
...
scp -i ~/.ssh/trading_vm_secure \
    ubuntu@80.225.198.195:~/trading-system/reports/daily/*.xlsx \
    reports/daily/
```
