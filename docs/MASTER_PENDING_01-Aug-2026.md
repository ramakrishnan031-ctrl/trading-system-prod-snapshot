...
## A2 — ~~TUE 4-AUG~~ ⭐ **WED 5-AUG: THE FLAG FLIP** ~~`<PENDING>`~~ ~~✅ **`<DEPLOYED>` 04-Aug 19:08:09**~~ ⭐⭐ **`<DEPLOYED — CAPABILITY VERIFIED LIVE 05-Aug 08:15; OUTCOME UNVERIFIED>`** — the first irreversible step, taken *(⛔ the compound label is deliberate: `<VERIFIED LIVE>` is reserved for a real CNC order + its GTT at the broker, and **`will_trade` is a permission, not an order** — see the BOOT PASS in the currency block)*
...
>
> ### 🔴 THE THREE ITEMS OPEN *TODAY* — ⛔ (a) and (b) CANNOT BE RE-RUN; (c) IS §C.5's DATED ROW
> **(a) DO THE THREE POSITIONAL STRATEGIES ACTUALLY PLACE ORDERS?** Entry window opened **10:00**. Watch: the first **CNC order** · its **GTT** · ⭐⭐ **the first `gtt_state` row this system has ever written in production** (the table holds **0 rows**).
...
> **(b) ~17:35 — THE SHUTDOWN CENSUS. ⛔ THE ONE ARTIFACT THAT CANNOT BE RE-RUN** (it emits at `_shutdown()` and is gone if the service is disturbed). 🔴 **If delivery TRADED and `cnc_gtt_placer`/`cnc_gtt_monitor` still read `acted 0`, THAT IS A FINDING** — R4 moved both to `expected-event-driven` precisely so this reads as a mismatch rather than as design.
> **(c) 🔴 WED EVENING, BEFORE THU 08:15 — THE CHECK1 GATE.** ⛔ Not restated here; it is **§C.5's first row, in full, with both routes and all three branches.** ⭐ **Cross-referenced rather than copied (§G3)** — a gate written twice is a gate that gets updated once.
> ⚠️ **AND THE TWO NON-INTERVENTIONS, because on flip day the temptation runs the other way:** a **CNC position still open after 15:17 is CORRECT** (EOD6 never touches CNC) ⇒ ⛔ **no intervention** · and **THU 6-AUG** is when the holdings-blindness bites (**11 positions-only readers, 6 correct BY ACCIDENT, 3 of those on the kill path** — §C.6 F1), which is a **Thursday** fact and must not be pulled forward into Wednesday's decisions.
