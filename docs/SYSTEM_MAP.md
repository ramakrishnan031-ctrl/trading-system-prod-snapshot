...
| `sr_detector/` | **SNR-DETECTOR-V1** shadow S&R detector (PURE pkg: core/+stdlib only; async non-gating observer; default-off `system.sr_detector.enabled`) | models, fetch (rate-limited fetch fn injected from main.py), pivots, zones (clusters + volume profile), confluence (multi-method/multi-TF → HIGH/MED/LOW + evidence), flags (BUYING_INTO_RESISTANCE etc + retest proposal), detector (single bg worker → `sr_detector_results`) |
...
### Database  (`data_store/`)
- `trading_system.db` (~161 MB on VM) — **MAIN** DB (schema **v37**; **v37 added `sr_detector_results`** — SNR-DETECTOR-V1 shadow S&R log (one row per placed candidate; actual_*/win_loss/pnl EOD-backfilled), pure addition; **v36 added `gtt_state`** — durable one-OCO-GTT-per-CNC-trade source of truth, SLICE2.5-P2, pure addition; v35 added trades.tgt_risk_reward_applied/exits_verified; v33 preflight_*; v32 tolerance_*; v30 added trades.needs_tgt_retry/…; v31
