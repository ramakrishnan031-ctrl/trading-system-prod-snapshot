...
# Last updated: 2026-06-26 by Claude Code (Opus) — SLICE2.5-PHASE-3: (A) separate delivery (CNC) count caps (risk.max_open_delivery_positions=3/max_daily_delivery_trades=5 + product-keyed counts; risk_engine OPEN_POSITIONS/DAILY_TRADES branch on bucket=="positional", intraday branch byte-unchanged) + (B) conditional capital allocation flag (capital.conditional_allocation_enabled=false; pure resolve_bucket_allocation drives the effective bucket split; FundManager unchanged). BOTH INERT (flag off + force_intraday_only coercion → caps never hit, counts 0); no schema; branch phase3-delivery-caps-conditional-capital-26jun (NOT deployed). [prev: 2026-06-26 FIX-183 orphan-GTT ADOPTION DEPLOYED to main af4b784 (holiday self-exit, real boot Mon 08:15); 2026-06-25 SLICE2.5-P2 durable CNC overnight-GTT schema v36 DEPLOYED bad0aad] [prev: 2026-06-25 SLICE2.5-P2 durable CNC overnight-GTT (schema v36 gtt_state) + CncGttMonitor reconcile/GTT_EXIT/recreate/F6/qty-mismatch/soft-kill + 15-min in-hours monitor + R2 guard split + delivery-trade exclusion + GTT-exempt-from-sweeps; DEPLOYED to main bad0aad 25-Jun]
