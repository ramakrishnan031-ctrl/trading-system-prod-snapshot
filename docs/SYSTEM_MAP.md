...
| `sr_detector/` | **SNR-DETECTOR-V1** shadow S&R detector + **SNR-V2 Phase A** WAIT_FOR_RETEST (PURE pkg: core/+stdlib only; default-off `wait_for_retest_enabled`) | V1: models, fetch, pivots, zones, confluence, flags, detector (bg worker → `sr_detector_results`). V2: `zone_builder` (DRY zone-build shared with V1), `zone_cache` (sync hot-path read), `zone_warmer` (daemon), `retest_confirm` (pure state machine). The retest monitor/diverter live in `screening/retest_monitor.py` |
