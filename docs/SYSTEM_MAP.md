...
- `docs/design/p1_eod_broker_reconcile_build_design_02jul2026.md` — **BUILD DESIGN (read-only, no code)** for P1: a NEW standalone `scripts/eod_broker_reconcile.py` @15:58 (own creds via `reconcile_positions._resolve_credentials` → runs even if the process was DOWN at 15:17) that queries the broker (`get_positions`/`get_all_orders`/`get_margins`/day-P&L), compares to local, and emits **VERIFIED / ISSUES / UNVERIFIED** (broker-unreachable ⇒ UNVERIFIED, never false-pass) — REPLACING `eod_verify`'s local-only false-VERIFY. Subsumes the orphaned `reconcile_pnl` (queries the correct `system_pnl`/`broker_pnl` cols + real creds) and CONSUMES `reconcile_positions` (a VERIFIED can't contradict its 15:45 CRITICAL). 4 detections (orphan positions · capital drift · ledger drift · manual broker actions + the exit-reason TGT/MANUAL label refinement). **DETECT+VERIFY+ALERT only — no auto-fix (that's P3).** New `eod_broker_reconciliation` verdict table (pure-add); one code path (paper = adapter-sim self-consistency, labeled); SHADOW-first (`eod_reconcile.authoritative=false` → run alongside eod_verify, observe, then cutover + retire eod_verify). memory `p1_eod_broker_reconcile_build_design_02jul`.
