...
| [11](11_absolute_kill_ladder.md) | **Absolute rupee kill ladder** — drift handler + reconciler tolerances | capital-posture | ⭐ **PARTLY DECIDED (28-Jul)** — posture SETTLED **absolute** (Rama); options C/D closed. **VALUES still open**: `soft_kill` 1,000 → **1,500** is a kill-path re-tune | ⛔ **no value changed.** Needs the careful loop + **its own single-variable boot ⇒ earliest slot is AFTER the 4-Aug flag flip**. ⭐ MEASURED 28-Jul over 22 files / 1.54 M lines: **the ladder has NEVER fired** — 2 drift events total, both from a **non-escalating** source at ₹10,000 (4× HARD), logged INFO by design. ⛔ A realised-drift **distribution is NOT obtainable** — the noise tier logs at DEBUG. 2 questions open for Rama (§6) · `../audit/capital_figure_sweep_28jul2026.md` §4 |
